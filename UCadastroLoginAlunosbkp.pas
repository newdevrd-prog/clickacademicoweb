...
var
  HTTP: TNetHTTPClient;
  JSONDoc, FieldsObj, Item: TJSONObject;
...
begin
  ...
  try
    ...
    try
      ...
      FieldsObj.AddPair('codigo_aluno', Item);
      
      Item := TJSONObject.Create;
      Item.AddPair('stringValue', lblNomeAluno.Caption.Replace('Nome do Aluno: ', ''));
      FieldsObj.AddPair('nome_aluno', Item);
      
      Item := TJSONObject.Create;
      Item.AddPair('stringValue', edtUsuario.Text);
      FieldsObj.AddPair('usuario', Item);
      
      ValorAtivo := IfThen(chkAtivo.Checked, 1, 0);
      
      Item := TJSONObject.Create;
      Item.AddPair('integerValue', ValorAtivo.ToString);
      FieldsObj.AddPair('ativo', Item);
      
      Item := TJSONObject.Create;
      Item.AddPair('timestampValue', FormatDateTime('yyyy-mm-dd"T"hh:nn:ss"Z"', Now));
      FieldsObj.AddPair('data_cadastro', Item);
      ...
      try
        ...
        begin
          ...
          try
            DocId := JSONRes.GetValue('name')?.Value;
            if DocId <> '' then
            begin
